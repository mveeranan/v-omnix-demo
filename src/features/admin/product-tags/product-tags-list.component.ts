    ...
    <app-admin-page-shell eyebrow="Catalog" title="Product tags" description="Tags for filtering and organizing products.">
      ...
      @if (!formOpen()) {
        ...
        <div class="mb-4 flex justify-end">
          ...
          <button type="button" class="admin-section-action-btn rounded-lg px-4 py-2 text-sm" (click)="openCreate()">
      ...
      @if (formOpen()) {
        ...
        <form class="admin-glass-card mb-4 space-y-4 rounded-xl p-6" [formGroup]="form" (ngSubmit)="save()">
          ...
          <h3 class="text-lg font-semibold">{{ editingId() ? 'Edit tag' : 'New tag' }}</h3>
          ...
          <div class="flex justify-end gap-2">
            ...
            <button type="submit" class="admin-section-action-btn rounded-lg px-4 py-2 text-sm" [disabled]="form.invalid">
      ...
      @if (!formOpen()) {
        ...
        @if (loading()) {
          ...
          <div class="admin-glass-card rounded-xl p-8 text-center">
            ...
            <button type="button" class="admin-section-action-btn mt-4 rounded-lg px-4 py-2 text-sm" (click)="openCreate()">
                  ...
                  <tr class="admin-data-table__row">
                    ...
                    <td class="admin-data-table__col-actions">
                      ...
                      <div class="admin-data-table__actions">
                        ...
                        <app-admin-table-action label="Delete" variant="delete" (action)="confirmDelete(t)" />
...
export class ProductTagsListComponent implements OnInit {
  ...
  readonly deleteTarget = signal<ProductTagDto | null>(null);
  ...
  openEdit(tag: ProductTagDto): void {
  ...
  save(): void {
    ...
    req$.subscribe({
      ...
      next: () => {
        ...
        this.notifications.success(id ? 'Tag updated' : 'Tag created');
  ...
  confirmDelete(tag: ProductTagDto): void {
  ...
  doDelete(): void {
    ...
    this.api.delete(tag.id).subscribe({
      ...
      next: () => {
        ...
        this.notifications.success('Tag deleted');
